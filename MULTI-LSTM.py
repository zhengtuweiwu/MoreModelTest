...
def train(train_generator,train_size,input_num,dims_num):
    print("Start Train! ")
    start=time.time()
    layer1=LSTM(hidden_size,activation="tanh",input_dim=dims_num, input_length=input_num,input_shape=(input_num,dims_num),return_sequences=True)
    ...
    model.save_weights('file\\LSTM-'+str(hidden_size)+'-'+str(hidden_layer)+'-2.h5')
    model_json = model.to_json()
    with open('file\\LSTM-'+str(hidden_size)+'-'+str(hidden_layer)+'-2.json', 'w') as file:
...
def test(test_generator,test_size,input_num,dims_num,batch_size):
    with open('file\\LSTM-'+str(hidden_size)+'-'+str(hidden_layer)+'-2.json', 'r') as file:
        json_string = file.read()
    new_model = keras.models.model_from_json(json_string)
    new_model.load_weights('file\\LSTM-'+str(hidden_size)+'-'+str(hidden_layer)+'-2.h5', by_name=True)
    labels_pre=[]
    labels_true=[]
