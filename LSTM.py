...
def train(train_generator,train_size,input_num,dims_num):
    print("Start Train! ")
    start=time.time()
    layer1=LSTM(128,activation="tanh",input_dim=dims_num, input_length=input_num,input_shape=(input_num,dims_num))
    ...
    model.save_weights('file\\LSTM.h5')
    model_json = model.to_json()
    with open('file\\LSTM.json', 'w') as file:
...
def test(test_generator,test_size,input_num,dims_num,batch_size):
    with open('file\\LSTM.json', 'r') as file:
        json_string = file.read()
    new_model = keras.models.model_from_json(json_string)
    new_model.load_weights('file\\LSTM.h5', by_name=True)
    labels_pre=[]
    labels_true=[]
