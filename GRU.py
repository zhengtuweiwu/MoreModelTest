...
def train(train_generator,train_size,input_num,dims_num):
    print("Start Train! ")
    start=time.time()
    layer1=GRU(128,activation="relu",input_dim=dims_num, input_length=input_num,input_shape=(input_num,dims_num))
    ...
    model.save_weights('file\\GRU.h5')
    model_json = model.to_json()
    with open('file\\GRU.json', 'w') as file:
...
def test(test_generator,test_size,input_num,dims_num,batch_size):
    with open('file\\GRU.json', 'r') as file:
        json_string = file.read()
    new_model = keras.models.model_from_json(json_string)
    new_model.load_weights('file\\GRU.h5', by_name=True)
    labels_pre=[]
    labels_true=[]
